...
class GridSearchWrapper:
    ...
    def setup(self):
        ...
        gs_params = GridSearch.Schema().dump(self.__estimator_handler.grid_search)
    ...
    def fit(self, df, y):
        self.__column_names = ", ".join(df.columns.values)
        self.__grid_search.fit(df, y)
    ...
    def __create_folder_for_estimator_saving(self, grid_search):
        ...
        path_to_folder = Path(
            self.__config.get_model_save_path()
        ) / folder_name
    ...
    def __save_cv_results(self, grid_search, path_to_folder):
        ...
        estimators_string = " ".join([str(elem) for elem in self.__config.get_estimator_handler().estimators])
        log_file.write("\ndataframe columns: {}".format(self.__column_names))
        log_file.write("\ndatabase limit: {}".format(self.__config.get_db_limit()))
        log_file.write("\nbest_parameter: {}".format(grid_search.best_params_))
        log_file.write("\npipeline: {}".format(self.__config.get_estimator_handler().pipelines))
